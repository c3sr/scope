...
#include "numamemcpy-duplex/args.hpp" // need to make file
...
static void DUPLEX_Memcpy_HostToGPU(benchmark::State &state) {
  ...
  numa_bind_node(numa); //do I need this line
//Do i need to rest numa
/*  if (PRINT_IF_ERROR(utils::cuda_reset_device(gpu0))) {
  ...
  // There are two copies, one numa -> gpu1, one gpu1 -> numa
...
*/
  ...
  // create a source and destination for second copy: gpu1 -> host
  if (PRINT_IF_ERROR(cudaSetDevice(gpu1))) {
    state.SkipWithError(NAME " failed to set device");
    return;
  }
