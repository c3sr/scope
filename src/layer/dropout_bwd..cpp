...
static void LAYER_CUDNN_DROPOUT_BACKWARD_INT8(benchmark::State& state) {
  CUDNN_Impl<int8_t>(state);
}

static void LAYER_CUDNN_DROPOUT_BACKWARD_INT32(benchmark::State& state) {
  CUDNN_Impl<int32_t>(state);
}

static void LAYER_CUDNN_DROPOUT_BACKWARD_HALF(benchmark::State& state) {
  CUDNN_Impl<__half>(state);
}

static void LAYER_CUDNN_DROPOUT_BACKWARD_FLOAT(benchmark::State& state) {
  CUDNN_Impl<float>(state);
}

static void LAYER_CUDNN_DROPOUT_BACKWARD_DOUBLE(benchmark::State& state) {
  CUDNN_Impl<double>(state);
}

#define CONV_PROBLEMS INFERENCE_SERVER_CONV_PROBLEMS

BENCHMARK(LAYER_CUDNN_DROPOUT_BACKWARD_INT8)->INFERENCE_SERVER_CONV_PROBLEMS()->UseManualTime();
BENCHMARK(LAYER_CUDNN_DROPOUT_BACKWARD_INT32)->INFERENCE_SERVER_CONV_PROBLEMS()->UseManualTime();
BENCHMARK(LAYER_CUDNN_DROPOUT_BACKWARD_HALF)->INFERENCE_SERVER_CONV_PROBLEMS()->UseManualTime();
BENCHMARK(LAYER_CUDNN_DROPOUT_BACKWARD_FLOAT)->INFERENCE_SERVER_CONV_PROBLEMS()->UseManualTime();
BENCHMARK(LAYER_CUDNN_DROPOUT_BACKWARD_DOUBLE)->INFERENCE_SERVER_CONV_PROBLEMS()->UseManualTime();
