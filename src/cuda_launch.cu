...
CUDA_LAUNCH_IMPLEMENTATION_STRING(const CUDA_LAUNCH_IMPLEMENTATION impl) {
  switch (impl) {
...
template <typename T,  int ITERATION_COUNT , int BLOCK_SIZE > __global__ void cuda_add_two_kernel(T *vec, size_t len) {
...
template <typename T, int ITERATION_COUNT , int BLOCK_SIZE > __global__ void cuda_relu_kernel(T *vec, size_t len) {
  int index = threadIdx.x + blockIdx.x * BLOCK_SIZE;
  if (index < len) {
...
template <CUDA_LAUNCH_IMPLEMENTATION IMPLEMENTATION , typename T, int LAUNCH_COUNT = 1, int ITERATION_COUNT = 1, int BLOCK_SIZE = 128>
static void CUDA_LAUNCH(benchmark::State &state) {

  const std::string IMPLEMNTATION_NAME =
      CUDA_LAUNCH_IMPLEMENTATION_STRING(IMPLEMENTATION);
  ...
  if (cuda_err != cudaSuccess) {
    LOG(critical,
        "CUDA/LAUNCH/{} device memory allocation failed for vector A", IMPLEMNTATION_NAME);
    return;
  }
  defer(cudaFree(d_a));
...
switch ( IMPLEMENTATION) {
  case CUDA_LAUNCH_IMPLEMENTATION::EMPTY:
    cuda_empty_kernel<T, ITERATION_COUNT, BLOCK_SIZE>
        <<<gridDim, blockDim>>>(d_a, N);
        break ;
  case CUDA_LAUNCH_IMPLEMENTATION::ADDTWO:
    cuda_add_two_kernel<T, ITERATION_COUNT, BLOCK_SIZE>
        <<<gridDim, blockDim>>>(d_a, N);
        break ;
  case CUDA_LAUNCH_IMPLEMENTATION::RELU:
    cuda_relu_kernel<T, ITERATION_COUNT, BLOCK_SIZE>
        <<<gridDim, blockDim>>>(d_a, N);
        break ;
}
    ...
    if (cuda_err = CUDA_PERROR(cudaEventElapsedTime(&msecTotal, start, stop))) {
      ...
      state.SkipWithError(
          fmt::format("CUDA/LAUNCH/{} failed to get elapsed time",
                      IMPLEMNTATION_NAME)
              .c_str());
